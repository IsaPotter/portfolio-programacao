          ...
          <div className="flex gap-3">
            {liveUrl && (
              <a
                ...
                className="bg-white p-3 rounded-full text-slate-900 hover:bg-violet-500 hover:text-white transition-colors"
              >
                <svg
      ...
      <div className="p-8 flex flex-col flex-grow">
        <div className="flex flex-wrap gap-2 mb-4">
          {tags.map((tag) => (
            <span
              key={tag}
              className="bg-violet-50 text-violet-700 text-[10px] font-black uppercase tracking-widest px-3 py-1 rounded-full border border-violet-100"
            >
              {tag}
            </span>
          ))}
        ...
        <div className="pt-6 border-t border-slate-50 mt-auto">
          <a
            href={liveUrl}
            className="inline-flex items-center text-slate-900 font-black text-sm uppercase tracking-wider group/link"
          >
            Ver detalhes
            <svg
              xmlns="http://www.w3.org/2000/svg"
              className="h-4 w-4 ml-2 group-hover/link:translate-x-2 transition-transform"
              fill="none"
              viewBox="0 0 24 24"
              stroke="currentColor"
            >
